...
>
> Bob's current version is [`intent-mapping.md`](./intent-mapping.md). That's the canonical one.
> This file exists so we can diff and recover anything Bob's rewrite dropped.
> Run `git diff docs/specs/intent-mapping.v0.md docs/specs/intent-mapping.md` to compare.
